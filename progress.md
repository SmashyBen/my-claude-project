...
- **Session 14 — 2026-06-13:** Major UI overhaul. EQ expanded to 32 ISO 1/3-octave bands (16Hz–20kHz); fader block fixed-width and centred regardless of window size; caps narrowed to 8px fixed; faders glow amber when EQ is ON (not bypassed). MarkingKnobLAF added: draws knob body at smaller inset radius, tick marks and labels around the arc. All delay/reverb knobs get scale markings: Delay Time (ms marks every 200ms, or note divisions in sync mode); Feedback/Mix (0–50–100 marks); LPF (200/6.8k/13k/20k); Reverb Pre (0–50–100ms); Size (0–50–100); Mix and LPF same. Delay/Reverb knob layout changed to zigzag (4 knobs in a row, alternating high/low positions). Delay sync now keeps Time knob visible in sync mode — knob range switches to 0–6 with note-division labels; DLY/Sync/P-P buttons all moved to top row. Delay feedback range changed to 0–100. Build clean. Late in session: LPF and Mix knob positions swapped (LPF now 3rd, Mix now 4th) for both Delay and Reverb; knob sizes increased ~50% (maxKs 70→95, fixed height formula). Build clean, visually verified.
