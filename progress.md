...
- **Started:** 2026-06-09
- **Last updated:** 2026-06-14 (session 16)
...
- **Session 17 — 2026-06-14:** WOoKOO Writer built — retro turtle graphics in a single HTML file. VGA 16-colour pen (SETPC 0–15, wraps in REPEAT so colours cycle). Animated drawing with speed slider and STOP button. REPCOUNT support inside REPEAT. Light (cream) and dark (black) mode, switchable mid-drawing. Pig-on-fish pixel art sprite replaces triangle turtle — flips horizontally when moving right so pig stays on top, rotates with travel direction. Sprite white background stripped to transparent; green variant for dark mode. Repo: github.com/SmashyBen/wookoo-writer (private). Smash Box v1.1 also pushed to github.com/SmashyBen/smash-box (private), Becky invited. Installer binaries removed from git tracking.
- **Session 16 — 2026-06-14:** Colour scheme feature complete. Three schemes (Purple default, Green, Greyscale) selectable in the Audio Setup dialog. All button glows, knob arcs, fader fills, meter mid-zone, file browser highlights and label colours read from AppColourScheme::current at paint time. Artwork images are tinted on load (green wash / greyscale conversion). Choice persists across restarts via AppData/SmashBox/colourScheme.txt. Build clean (2fdf48d).
